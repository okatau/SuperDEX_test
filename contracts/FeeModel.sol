...
contract FeeModel is AugustusStorage {
    ...
    function takeFromTokenFee(
        ...
        (uint256 partnerShare, uint256 paraswapShare) = _calcFixedFees(
            fromAmount,
            fixedFeeBps
        );
        return
    ...
    function takeFromTokenFeeAndTransfer(
        ...
        if (fixedFeeBps != 0) {
            ...
            remainingAmount = _distributeFees(
                remainingAmount,
                fromToken,
                partner,
                partnerShare,
    ...
    function takeFromTokenFeeSlippageAndTransfer(
        ...
        uint256 fixedFeeBps = _getFixedFeeBps(partner, feePercent);
        uint256 slippage = _calcSlippage(
            fixedFeeBps,
            expectedAmount,
        ...
        if (slippage != 0) {
            (uint256 partnerShare2, uint256 paraswapShare2) = _calcSlippageFees(
                slippage,
                partner,
                feePercent
            );
    ...
    function takeToTokenFeeSlippageAndTransfer(
        ...
        Utils.transferTokens(toToken, beneficiary, feePaid);
        return feePaid;
    }
    ...
    function takeToTokenFeeAndTransfer(
        ...
        if (fixedFeeBps != 0) {
            ...
            receivedAmount = _distributeFees(
                receivedAmount,
                toToken,
                partner,
                partnerShare,
    ...
    function _getFixedFeeBps(address partner, uint256 feePercent)
        private
        view
        returns (uint256 fixedFeeBps)
    {
    ...
    function _calcSlippage(
        ...
        return
            (fixedFeeBps <= 50 && positiveAmount > negativeAmount)
                ? positiveAmount.sub(negativeAmount)
                : 0;
    }
    ...
    function _distributeFees(
        ...
        require(
            totalFees <= currentBalance,
            "Insufficient balance to pay for fees"
        );
    ...
    function _isTakeFeeFromSrcToken(uint256 feePercent)
        internal
        pure
        returns (bool)
    {
