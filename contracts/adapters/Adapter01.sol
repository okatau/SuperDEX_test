...
abstract contract UniswapV2Router is WethProvider{
    struct UniswapV2DataCustom{
        address[] path;
        uint256 amountOutMin;
        address toApprove;
    }
    ...
    {
        (address[] memory _path, uint256 _amountOutMin, address _toApprove) = abi.decode(payload, (address[], uint256, address)); 
        
        IERC20(_path[0]).approve(_toApprove, amountIn);
        
        IUniswapV2Router(exchange).swapOnUniswap(amountIn, _amountOutMin, _path);
    }
}
...
contract Adapter01 is IAdapter, NewUniswapV2, ZeroxV4, ZeroxV2, Curve, Balancer, UniswapV3, Aavee2, OneInchPool, MStable, CurveV2, UniswapV2Router {
    using SafeMath for uint256;
    ...
    function _approve(address _token, address _to, uint256 _amount) public {
